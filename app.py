...
def eliminar_registro(modelo, registro_id, tabla_nombre, usuario_id):
    """Función genérica para eliminar un registro con auditoría"""
...
menu_opciones = ["Dashboard", "Proyectos", "Partidas Presupuesto", "Gastos", "Pagos", "Proveedores", "Conciliación", "Reportes"]
if st.session_state.rol_actual == "Gerencia":
    menu_opciones.append("Usuarios")
menu = st.sidebar.radio("Navegación", menu_opciones)

# --- FUNCIONES AUXILIARES PARA KPI (RENTABILIDAD REAL) ---
...
# ============================================
# MÓDULO: PROYECTOS (CON BÚSQUEDA, DETALLE Y ELIMINACIÓN)
# ============================================
elif menu == "Proyectos":
    ...
    with st.expander("➕ Nuevo Proyecto", expanded=False):
        with st.form("nuevo_proyecto"):
            nombre = st.text_input("Nombre del Proyecto/Sitio")
            ...
            if submitted and nombre:
                nuevo = Proyecto(
                    nombre=nombre, ubicacion=ubicacion, cliente=cliente,
                    n_requerimiento=n_requerimiento, acta_conciliacion=acta,
                    estado=estado, created_by=usuario.id
                )
                db.add(nuevo)
                db.commit()
    ...
    if not proyectos:
        ...
        for p in proyectos:
            ...
            with st.container(border=True):
                ...
                col2.metric("Ingresos", f"${kpi['ingresos']:,.0f}")
                col3.metric("Costos", f"${kpi['costos']:,.0f}")
                col4.metric("Rentabilidad", f"{kpi['rentabilidad']:.1f}%")
                
                with col5:
                    st.write("")
                    st.write("")
                    if st.button(f"📊 Detalle", key=f"detalle_{p.id}"):
                        st.session_state[f"detalle_{p.id}"] = not st.session_state.get(f"detalle_{p.id}", False)
                        st.rerun()

                if st.session_state.rol_actual in ["Gerencia", "Auxiliar Contable"]:
                    if st.button(f"✏️ Editar", key=f"edit_{p.id}"):
                ...
                if st.session_state.rol_actual in ["Gerencia", "Auxiliar Contable"]:
                    if st.button(f"🗑️ Eliminar", key=f"delete_{p.id}"):
                        ...
                        if partidas > 0 or gastos > 0:
                            st.error(f"No se puede eliminar el proyecto porque tiene {partidas} partidas y {gastos} gastos asociados.")
                        else:
                            if st.button(f"Confirmar eliminación de {p.nombre}", key=f"confirm_delete_{p.id}"):
                                db.delete(p)
                                db.commit()
                ...
                if st.session_state.get(f"detalle_{p.id}", False):
                    st.markdown("---")
                    ...
                    if st.button("❌ Cerrar Detalle", key=f"cerrar_{p.id}"):
                        st.session_state[f"detalle_{p.id}"] = False
                        st.rerun()

# ============================================
# MÓDULO: PARTIDAS PRESUPUESTO (CON ELIMINACIÓN)
# ============================================
...
# ============================================
# MÓDULO: GASTOS (CON FILTROS, CARRITO Y ELIMINACIÓN)
# ============================================
elif menu == "Gastos":
                ...
                for gasto in gastos:
                    ...
                    with st.container(border=True):
                        ...
                        if pagos:
                            ...
                            for pago in pagos:
                                col_p1, col_p2, col_p3, col_p4, col_p5 = st.columns([1.5, 1.5, 1.5, 2, 0.5])
                                col_p1.write(pago.fecha.strftime("%Y-%m-%d"))
                                col_p2.write(pago.tipo)
                                col_p3.write(f"${pago.monto:,.0f}")
                                col_p4.write(pago.numero_factura or "")
                                with col_p5:
                                    if st.session_state.rol_actual in ["Gerencia", "Auxiliar Contable"]:
                        ...
                        if saldo > 0:
                            with st.expander("Registrar Pago"):
                                with st.form(f"pago_{gasto.id}"):
                                    tipo = st.selectbox("Tipo", ["Factura", "Anticipo"])
...
# ============================================
# MÓDULO: PROVEEDORES (CON ELIMINACIÓN)
# ============================================
...
# ===============================
# MÓDULO: CONCILIACIÓN (CORREGIDO - SIN BOTONES DENTRO DE FORMULARIOS)
# ===============================
    ...
    if st.session_state.rol_actual not in ["Gerencia", "Auxiliar Contable"]:
        ...
        if tipo_conciliacion == "💳 Gastos (Pagos a Proveedores)":
            ...
            if not gastos:
                ...
                # Tabla con selección múltiple nativa
                ...
                if gastos:
                    ...
                    if gasto:
                        # Mostrar archivo actual y botón eliminar FUERA del formulario
        ...
        # ============================================
            ...
            with tab1:
                ...
                if not partidas:
                    ...
                    # --- REGISTRO DE COBRO MANUAL (DESDE SELECCIÓN) ---
                    if ids_seleccionados:
                        st.markdown("---")
...
# ============================================
# MÓDULO: REPORTES (SÁBANA ÚNICA Y MÚLTIPLES HOJAS)
# ============================================
elif menu == "Reportes":
    ...
    with st.expander("🔍 Filtros del Reporte", expanded=True):
        ...
        with col_f2:
            # Selección múltiple de proyectos
    ...
    def aplicar_filtros(query, tabla_fecha, fecha_desde, fecha_hasta):
        """Aplica filtros de fecha y proyecto a una query"""
        if fecha_desde:
            query = query.filter(tabla_fecha >= fecha_desde)
        if fecha_hasta:
            query = query.filter(tabla_fecha <= fecha_hasta + datetime.timedelta(days=1))
    ...
    def obtener_proyectos_filtrados():
        """Obtiene lista de IDs de proyectos según selección"""
    ...
    def calcular_kpi_proyecto(proyecto_id):
        """Calcula KPIs para un proyecto (versión mejorada para reportes)"""
        # Ingresos (suma de partidas)
        ...
        # Pagos (suma de pagos a proveedores)
    ...
    def generar_sabana_unica(proyectos_ids):
        """Genera la sábana única con todas las transacciones"""
        ...
        # --- PARTIDAS (Ingresos presupuestados) ---
        if incluir_partidas:
            ...
            for p in partidas:
                proyecto = db.query(Proyecto).filter(Proyecto.id == p.proyecto_id).first()
                kpi = calcular_kpi_proyecto(p.proyecto_id) if proyecto else None
        ...
        if incluir_gastos:
            ...
            for g in gastos:
                proyecto = db.query(Proyecto).filter(Proyecto.id == g.proyecto_id).first()
                proveedor = db.query(Proveedor).filter(Proveedor.id == g.proveedor_id).first()
                kpi = calcular_kpi_proyecto(g.proyecto_id) if proyecto else None
                
                # Calcular días de atraso (desde fecha de creación)
        ...
        if incluir_pagos:
            ...
            for pag in pagos:
                ...
                kpi = calcular_kpi_proyecto(gasto.proyecto_id) if proyecto else None
                
                # Calcular días de atraso (desde fecha de pago hasta hoy)
        ...
        if incluir_cobros:
            ...
            for cob in cobros:
                ...
                kpi = calcular_kpi_proyecto(cob.proyecto_id) if proyecto else None
                
                # Calcular días de atraso (desde fecha de cobro hasta hoy)
